...
DownloadThread::DownloadThread(QObject *parent) : QObject(parent)
{
    ...
    this->moveToThread(m_thread); // 移动QObejct的子类到线程中执行
    m_thread->start();
...
void DownloadThread::get()
{
    ...
    m_request.setUrl(m_infos.first().getUrl());   // 下载列表中第一个
...
void DownloadThread::on_finished(QNetworkReply *reply)
{
    ...
    {
         auto img = QImage(reply->readAll()).convertToFormat(QImage::Format_RGB888);
         info.texture->setData(0, info.layer, QOpenGLTexture::RGB, QOpenGLTexture::UInt8, img.constBits());
